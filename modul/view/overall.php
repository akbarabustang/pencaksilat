      ...
      <div class="modal-body">
        <form class="form-horizontal" role="form" method="post" action="#">
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="kontingen" id="kontingen_pilih" required="on">
            <option>-silahkan pilih kontingen-</option>
            <?php 
              include_once "lib/config.php";
              $x  = new Database;
              $kelas_q    = "SELECT * FROM kontingen_all";
              $exec       = $x->custom_query($kelas_q);
              foreach ($exec as $kelas_nama) { ?>
                <option value="<?php echo $kelas_nama->isi_kontingen; ?>"><?php echo $kelas_nama->isi_kontingen; ?></option>
              <?php } ?>      
          ...
          <div class="form-group">
            ...
            <div class="col-md-6">
             <select class="form-control" name="perguruan" id="perguruan_pilih" required="on">
             <option>-silahkan pilih Asal Perguruan-</option>
            <?php 
              include_once "lib/config.php";
              $x  = new Database;
              $kelas_q    = "SELECT * FROM perguruan_all";
              $exec       = $x->custom_query($kelas_q);
              foreach ($exec as $kelas_nama) { ?>
                <option value="<?php echo $kelas_nama->isi_perguruan; ?>"><?php echo $kelas_nama->isi_perguruan; ?></option>
              <?php } ?>      
          ...
          <div class="form-group">
            ...
            <div class="col-md-6">
            <select class="form-control" name="kelas_pilih" id="kelas_pilih" required="on">
            ...
            <?php 
              ...
              $exec       = $x->custom_query($kelas_q);
              foreach ($exec as $kelas_nama) { ?>
                <option value="<?php echo $kelas_nama->isi_kelas; ?>"><?php echo $kelas_nama->isi_kelas; ?></option>
              <?php } ?>      
    ...
    ?>
     ...
     $("#overall").dataTable({
      ...
      $(document).ready(function() {
        ...
        $("#kelas_pilih" ).change(function() {
           var data = $("#kelas_pilih option:selected").text();
            if (data.indexOf("Putra") >= 0) {
              $('#jk').val('Putra');
            }else{
              $('#jk').val('Putri');
            }
